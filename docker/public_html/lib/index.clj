   ...
   [:html
    ...
    [:body
     [:div.page
      ...
      [:p
       "Don't forget to add your libary jars, add pods, and set their permissions appropriately. In this local docker development server you should be able to use Clojure, lein, and babashka to install the libs and pods you need. However in a production-grade shared host, that is usually not an option so you will need to jar or build those libraries locally then upload and use the static binary versions of pods and babashka as well."]]]])))
